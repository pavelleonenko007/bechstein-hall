			...
			foreach ( $footer_columns as $index => $footer_column ) :
				...
				<div id="w-node-e1be876e-05a4-9245-628d-78602bcc79a4-2bcc79a3" class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>">
					...
					foreach ( $column_elements as $column_element ) :
						?>
						<?php
						if ( 'big_link' === $column_element['acf_fc_layout'] ) :
							if ( ! bech_is_current_url( $column_element['url'] ) ) :
								?>
								<a href="<?php echo esc_url( $column_element['url'] ); ?>" class="link-foo-big"><?php echo esc_html( $column_element['link_title'] ); ?></a>
							<?php else : ?>
								<div class="link-foo-big w--current"><?php echo esc_html( $column_element['link_title'] ); ?></div>
							<?php endif; ?>
							<?php
							elseif ( 'small_link' === $column_element['acf_fc_layout'] ) :
								if ( ! bech_is_current_url( $column_element['url'] ) ) :
									?>
								<a href="<?php echo esc_url( $column_element['url'] ); ?>" class="link-foo-small <?php echo ! $column_element['show_on_mobile'] ? 'no-mob' : ''; ?>"><?php echo esc_html( $column_element['link_title'] ); ?></a>
								<?php else : ?>
								<div class="link-foo-small <?php echo ! $column_element['show_on_mobile'] ? 'no-mob' : ''; ?> w--current"><?php echo esc_html( $column_element['link_title'] ); ?></div>
							<?php endif; ?>
						<?php elseif ( 'separator' === $column_element['acf_fc_layout'] ) : ?>
							<div class="foo-marger"></div>
							<?php
							elseif ( 'card' === $column_element['acf_fc_layout'] ) :
								if ( ! bech_is_current_url( $column_element['url'] ) ) :
									?>
								<a href="<?php echo esc_url( $column_element['url'] ); ?>" class="link-foo-whats_last w-inline-block">
										<?php
						...
						<?php elseif ( 'desktop_hidden_links' === $column_element['acf_fc_layout'] ) : ?>
							<div class="foo-bottom no-pc">
								<?php
								foreach ( $column_element['hidden_desktop_links'] as $hidden_desktop_link ) :
									if ( ! bech_is_current_url( $hidden_desktop_link['url'] ) ) :
										?>
										<a href="<?php echo esc_url( $hidden_desktop_link['url'] ); ?>" class="link-foo-small _2"><?php echo esc_html( $hidden_desktop_link['title'] ); ?></a>
									<?php else : ?>
										<div class="link-foo-small _2 w--current"><?php echo esc_html( $hidden_desktop_link['title'] ); ?></div>
									<?php endif; ?>
								<?php endforeach; ?>
							</div>
		...
		<div class="footer-container bottom">
				...
				<div class="div-block-2">
					<div class="p-20-27">
						...
						if ( ! empty( $phone_number ) ) :
							?>
							<a href="<?php echo esc_url( bech_format_phone_to_link( $phone_number ) ); ?>"><?php echo esc_html( $phone_number ); ?></a>
						<?php endif; ?>
						<?php
			...
			<div class="footer-col _3">
				<div class="foo-bottomer">
					...
					<a href="https://www.ettyq.com/industries/creative-agency-for-arts-media-cultural-institutions" target="_blank" rel="nofollow" class="funk-link">Website made by ettyq</a>
				</div>
			</div>
		</div>
	</div>
