	...
	<header class="header">
		<div class="left-header">
			<?php if ( ! bech_is_current_url( home_url( '/' ) ) || is_search() ) : ?>
				<a href="<?php echo esc_url( get_home_url( '/' ) ); ?>" aria-current="page" class="brand w-nav-brand">
		...
		<div class="right-header">
			<nav role="navigation" class="nav-menu w-nav-menu">
				<?php foreach ( $main_menu_items as $main_menu_item ) : ?>
					<a <?php echo ! bech_is_current_url( $main_menu_item->url ) ? 'href="' . esc_url( $main_menu_item->url ) . '"' : ''; ?> title="Go to <?php echo esc_attr( $main_menu_item->title ); ?> page" class="navbar-links w-nav-link <?php echo bech_is_current_url( $main_menu_item->url ) ? 'w--current' : ''; ?>"><?php echo esc_html( $main_menu_item->title ); ?></a>
				<?php endforeach; ?>
			</nav>
			...
			<div class="r-head-sec">
				<div class="right-menuer">
					...
					<a bgline="1" href="<?php echo esc_url( get_the_permalink( $whats_on_page_id ) ); ?>" class="header-book-head-btn w-inline-block">
			...
			<form role="search" method="get" id="searchform" action="<?php echo esc_url( get_home_url( '/' ) ); ?>" class="searchhead">
	...
	<div class="burger-menu">
		<div class="foo-mom">
			<div class="footer-container top-container">
				<?php
				$footer_columns = get_field( 'footer', 'option' );
				foreach ( $footer_columns as $index => $footer_column ) :
					...
					?>
					<div id="w-node-e1be876e-05a4-9245-628d-78602bcc79a4-2bcc79a3" class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>">
						<?php
						$column_elements = $footer_column['footer_elements'];
						foreach ( $column_elements as $column_element ) :
							?>
							<?php if ( 'big_link' === $column_element['acf_fc_layout'] ) : ?>
								<a <?php echo ! bech_is_current_url( $column_element['url'] ) ? 'href="' . esc_url( $column_element['url'] ) . '"' : ''; ?> class="link-foo-big <?php echo bech_is_current_url( $column_element['url'] ) ? 'w--current' : ''; ?>"><?php echo esc_html( $column_element['link_title'] ); ?></a>
							<?php elseif ( 'small_link' === $column_element['acf_fc_layout'] ) : ?>
								<a <?php echo ! bech_is_current_url( $column_element['url'] ) ? 'href="' . esc_url( $column_element['url'] ) . '"' : ''; ?> class="link-foo-small <?php echo ! $column_element['show_on_mobile'] ? 'no-mob' : ''; ?><?php echo bech_is_current_url( $column_element['url'] ) ? 'w--current' : ''; ?>"><?php echo esc_html( $column_element['link_title'] ); ?></a>
							<?php elseif ( 'separator' === $column_element['acf_fc_layout'] ) : ?>
								<div class="foo-marger"></div>
							<?php elseif ( 'card' === $column_element['acf_fc_layout'] ) : ?>
								<a <?php echo ! bech_is_current_url( $column_element['url'] ) ? 'href="' . esc_url( $column_element['url'] ) . '"' : ''; ?> class="link-foo-whats_last w-inline-block <?php echo bech_is_current_url( $column_element['url'] ) ? 'w--current' : ''; ?>">
									<?php
									echo wp_get_attachment_image(
										$column_element['background_image']['ID'],
										'large',
										false,
										array(
									...
									?>
									<div class="text-block"><?php echo esc_html( $column_element['title'] ); ?></div>
								</a>
							<?php elseif ( 'desktop_hidden_links' === $column_element['acf_fc_layout'] ) : ?>
								<div class="foo-bottom no-pc">
									<?php foreach ( $column_element['hidden_desktop_links'] as $hidden_desktop_link ) : ?>
										<a <?php echo ! bech_is_current_url( $hidden_desktop_link['url'] ) ? 'href="' . esc_url( $hidden_desktop_link['url'] ) . '"' : ''; ?> class="link-foo-small _2 <?php echo bech_is_current_url( $hidden_desktop_link['url'] ) ? 'w--current' : ''; ?>"><?php echo esc_html( $hidden_desktop_link['title'] ); ?></a>
									<?php endforeach; ?>
								</div>
							<?php endif; ?>
						<?php endforeach; ?>
			...
			<div class="footer-container bottom">
				<div class="footer-col _1">
					<div class="div-block-2">
						<div class="p-20-27">
							<?php if ( ! empty( $phone ) ) : ?>
								<a href="<?php echo esc_url( bech_format_phone_to_link( $phone ) ); ?>"><?php echo esc_html( $phone ); ?></a>
							<?php endif; ?>
							<?php if ( ! empty( $email ) ) : ?>
								<a href="<?php echo esc_url( bech_format_email_link( $email ) ); ?>"><?php echo esc_html( $email ); ?></a>
							<?php endif; ?>
							London, W1U 2RJ, 18 – 22 Wigmore Street
						</div>
					</div>
					<?php
				...
				<div class="footer-col _3">
					<div class="foo-bottomer">
						...
						<a href="https://func.agency/" target="_blank" class="funk-link" rel="nofollow">Website made by Func. </a>
					</div>
				</div>
			</div>
		</div>
