...
const blackColorObserver = new IntersectionObserver(
	(entries, observer) => {
...
const whiteColorObserver = new IntersectionObserver(
	(entries, observer) => {
		entries.forEach((entry) => {
			if (entry.isIntersecting) {
...
document.querySelectorAll('.grey-z').forEach((trigger) => {
	blackColorObserver.observe(trigger);
});

document.querySelectorAll('.white-z').forEach((trigger) => {
	whiteColorObserver.observe(trigger);
});
...
class UserCart {
	constructor(userData = {}) {
		...
		this._logoutUrl = `https://tickets.${window.location.hostname}/en/logout/`;
		this._loginUrl = `https://tickets.${window.location.hostname}/en/login/`;
		this._checkoutUrl = `https://tickets.${window.location.hostname}/en/buyingflow/order/`;
...
const addTixFrame = () => {
	...
	frame.setAttribute(
		'src',
		`https://tickets.${window.location.hostname}/en/itix`
	);
...
const initTixSessions = () => {
	...
	const TIX_URL = `https://tickets.${window.location.hostname}/en/itix`;
	let isTixConnected = false;
	let tixConntectedInterval = null;
	let counter = 0;
	const getUserData = (event) => {
		if (event.origin !== `https://tickets.${window.location.hostname}`) return;
