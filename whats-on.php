<?php
	/**
	 * Template name: What's on Page Template
	 *
	 * @package 0.1.0
	 */
...
<html <?php language_attributes(); ?> data-wf-page="62bc3fe7d9cc6100802615a2" data-wf-site="62bc3fe7d9cc6134bf261592">
	...
	<body <?php body_class( 'body' ); ?>>
		<?php
			wp_body_open();
			get_template_part( 'inc/components/loader' );
		?>
					...
					<div class="catalog-row">
						<div class="filter-column">
							<form data-filter="form" class="filter-styk">
								<div class="search-filter">
									<input type="search" value="<?php echo esc_attr( get_search_query() ); ?>" class="search-line-input w-input" maxlength="256" name="s" placeholder="search by keyword…" id="s" required="" />
									...
									<div>
										...
										<div data-popup="filters" class="mobile-filter-popup">
											...
											<div class="mobile-filter-popup__body">
												...
												<div class="filters-div">
													...
													<?php foreach ( $event_tags as $index => $event_tag ) : ?>
														...
														<label class="w-checkbox cbx-mom">
															...
															<input type="checkbox" id="event_tag-<?php echo esc_attr( $event_tag->term_id ); ?>" name="event_tag[]" value="<?php echo esc_attr( $event_tag->slug ); ?>" style="opacity:0;position:absolute;z-index:-1" />
															<span class="filter-cbx ischbx w-form-label" for="event_tag-<?php echo esc_attr( $event_tag->term_id ); ?>"><?php echo esc_html( $event_tag->name ); ?></span>
														</label>
														<?php else : ?>
													...
													<?php if ( count( $event_tags ) > 3 ) : ?>
														<button type="button" class="show-all-btn show-all-btn--hide" data-button="show-all-filters" data-taxonomy="<?php echo esc_attr( $event_tags[0]->taxonomy ); ?>">show all</button>
														<?php endif; ?>
													</div>
												</div>
													<?php
												...
												?>
												<?php
												$genres = bech_get_custom_taxonomies( 'genres' );
												if ( ! empty( $genres ) ) :
													?>
												<div class="filters-div">
													<div class="filters-top-div">
													...
													foreach ( $genres as $index => $genre ) :
														...
														<label class="w-checkbox cbx-mom">
															<div class="w-checkbox-input w-checkbox-input--inputType-custom cbx"></div>
															<input data-filter="checkbox" type="checkbox" id="<?php echo $genre->taxonomy . '-' . $genre->term_id; ?>" name="genres[]" value="<?php echo $genre->slug; ?>" 
																<?php
																if ( isset( $_GET['genres'] ) && in_array( $genre->slug, $_GET['genres'] ) ) {
																	echo 'checked';}
																?>
												...
												?>
												<?php
												$instruments = bech_get_custom_taxonomies( 'instruments' );
												...
												<div class="filters-div">
													...
													foreach ( $instruments as $index => $instrument ) :
														...
														<label class="w-checkbox cbx-mom">
															<div class="w-checkbox-input w-checkbox-input--inputType-custom cbx"></div>
															<input data-filter="checkbox" type="checkbox" id="<?php echo $instrument->taxonomy . '-' . $instrument->term_id; ?>" name="instruments[]" value="<?php echo $instrument->slug; ?>" 
																<?php
																if ( isset( $_GET['instruments'] ) && in_array( $instrument->slug, $_GET['instruments'] ) ) {
																	echo 'checked';}
																?>
																style="opacity:0;position:absolute;z-index:-1" />
						...
						<div class="catalog-column">
							...
							<div id="selected-filters" class="filters-line-text <?php echo ! empty( $selected_filter_string ) ? 'filters-line-text--active' : ''; ?>">
								<div>you choose &#x27;<span data-filter="choosen"><?php echo $selected_filter_string; ?></span>’ in filters.</div>
								<a id="clear" href="#" class="clearfilter-btn"> clear filters</a>
							</div>
							<div id="tickets-container" class="cms-tems">
								...
								if ( ! empty( $_GET['from'] ) ) {
									$tickets_query_args['meta_query'][] = array(
										'key'     => '_bechtix_ticket_start_date',
										'value'   => sanitize_text_field( wp_unslash( $_GET['from'] ) ),
										'compare' => '>=',
										'type'    => 'DATE',
									);
								}
								...
								if ( $tickets_query->have_posts() ) :
									...
									// $tickets_query_args['paged'] = 2;

									// $next_page_tickets_query = new WP_Query( $tickets_query_args );

									// $next_page_tickets = bech_sort_tickets( $next_page_tickets_query->posts );
									// bech_dump( $tickets );
									// echo '======';
							...
							<div data-type="load_more" data-max-pages="<?php echo esc_attr( $tickets_query->max_num_pages ); ?>" style="width: 100%; height: 0px; background: green"></div>
						</div>
					</div>
							<?php
						endwhile;
